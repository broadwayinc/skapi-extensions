...
class Admin extends Skapi {
    ...
    async getServices(serviceId) {
        ...
        if (serviceId) {
            let services = await this.request('get-services', { service_id: serviceId }, { auth: true });
        ...
        this.serviceMap = [];
        if (!this.state.services) {
            let services = await this.request('get-services', null, { auth: true });
            let serviceList = {};
            for (let list in services) {
                for (let service of services[list]) {
                    for (let t in this.default_templates) {
    ...
    async createService(params) {
        ...
        const calculateDistance = (locale, region) => {
            const R = 6371e3;
            const φ1 = (locale.lat * Math.PI) / 180;
    ...
    fetchService(serviceId) {
        ...
        if (idx < 0)
            throw new SkapiError('Service does not exist.', { code: 'NOT_EXISTS' });
        return {
            index: idx,
    ...
    async deleteService(serviceId) {
        ...
        if (this.state.services[index].active > 0)
    ...
    async registerSubdomain(serviceId, params) {
        ...
        if (!serviceId)
            throw 'Serivce is required';
        if (!params?.subdomain)
            throw 'Subdomain is required';
        if (params.subdomain.length < 4 || invalid.includes(params.subdomain))
            throw 'Subdomain has been taken.';
        let { index, service } = this.fetchService(serviceId);
    ...
    async deleteSubdomain(serviceId) {
        await this.require(['admin']);
        if (!serviceId)
            throw 'Serivce is required';
        let { index, service } = this.fetchService(serviceId);
